function do_KP_decoding_hybrid_ROI_post_forward(SubjectID)
...
for ii = 1:146
    ...
    for roi = 1:148
        ...
        if ismember(roi,best_rois) ~=1
            ...
            for s = 1:length(stimdur)
                ...
                [kp_MEG_data, label] = get_kp_MEG_data_post(data_current,...
                    behav_all{sub,2},...
                    behav_all{sub,1},...
                    stimdur(s));
                ...
                best= best(1);
                acc_final(s) = acc(best);
                f_final{s} = f{best};
                f_mean_final(s)= f_mean(best);
                cm_final{s} = cm{best};
    ...
    mkdir(fullfile(pwd, 'output', 'Hybrid_ROI_Forward','results'))
    save(fullfile(pwd, 'output', 'Hybrid_ROI_Forward', 'results', SubjectID), 'result');
end
