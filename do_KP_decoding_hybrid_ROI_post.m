function do_KP_decoding_hybrid_ROI_post(SubjectID)
...
cd /path/to/ROI/results

files = dir('*.mat');
for i = 1:length(files)
    load(files(i).name)
    temp(i,:) = max(result.acc');
end

a = mean(temp);

[o,rois] = sort(a,'descend');

for roi = 1:8
    roi_n =  rois(1:roi);
    roi_data = [];
    for  n =  1: length(roi_n)
        roi_data = [roi_data; D{roi_n(n)}];
    end

    data_all = [data; roi_data]; % [all_parcels, top 8 rois ]
...
for s = 1:length(stimdur)
    ...
    [kp_MEG_data, label] = get_kp_MEG_data_post(data_all,...
        behav_all_S2r{sub,2},...
        behav_all_S2r{sub,1},...
        stimdur(s));
    ...
    folds = 8;
    cv = cvpartition(label,'KFold',folds);

    for fold = 1:folds
        ids{fold} = training(cv,fold);
        feat_train{fold} = feat(ids{fold},:);
        feat_test{fold} = feat(~ids{fold},:);
        lab_train{fold} = label(ids{fold});
        lab_test{fold} = label(~ids{fold});
    ...
    mkdir(fullfile(pwd, 'output', 'Hybrid','results'))
    save(fullfile(pwd, 'output', 'Hybrid', 'results', SubjectID), 'result');
end
